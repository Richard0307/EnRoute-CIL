...
class iSTATE_FARM(iIMAGENET_R):
    base_folder = 'statefarm_cl'
    im_size = 224
    nch = 3

    def load(self):
        split = 'train' if self.train or self.validation else 'test'
        split_root = os.path.join(self.root, self.base_folder, split)
        if not os.path.isdir(split_root):
            raise RuntimeError(
                f"StateFarm split not found: {split_root}. "
                "Prepare data under <dataroot>/statefarm_cl/{train,test}/class_x/..."
            )

        classes = sorted(
            item.name for item in os.scandir(split_root)
            if item.is_dir()
        )
